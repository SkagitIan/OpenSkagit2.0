...
class CloudinaryServiceTests(SimpleTestCase):
    @patch.dict("os.environ", {"CLOUDINARY_CLOUD_NAME": "demo", "CLOUDINARY_API": "key", "CLOUDINARY_SECRET": "secret"}, clear=False)
    ...
    def test_uploads_svg_with_deterministic_public_id(self, post, get):
        ...
        post.return_value = Mock(status_code=200, json=lambda: {"secure_url": "https://res.cloudinary.com/demo/image/upload/openskagit/visuals/maps/P123/map_hash.svg"})
        result = upload_generated_asset(b"<svg/>", asset_type="map", digest="hash", source_property_ids=["P123"], metadata={"aspect_ratio": "16:9"})
    ...
    def test_existing_asset_is_reused(self, get):
        get.return_value = Mock(status_code=200, json=lambda: {"secure_url": "https://example.test/existing", "width": 1920, "height": 1080})
        result = upload_generated_asset(b"svg", asset_type="infographic", digest="same", source_property_ids=[], metadata={})

(function () {
  ...
  function draw(data) {
    ...
    data.routes.forEach(function (route, index) {
      ...
      route.stops.forEach(function (stop) { var options = data.routes.filter(function (candidate) { return candidate.id !== route.id; }).map(function (candidate) { return '<option value="' + candidate.id + '">Route ' + candidate.route_number + '</option>'; }).join(""); html += '<li>' + String(stop.parcel_id || "") + (stop.address ? " — " + String(stop.address) : "") + ' <button type="button" class="routing-lock-stop" data-stop-id="' + stop.id + '">' + (stop.manually_locked ? "Unlock" : "Lock") + '</button> <select class="routing-move-target" data-stop-id="' + stop.id + '"><option value="">Move to…</option>' + options + '</select></li>'; });

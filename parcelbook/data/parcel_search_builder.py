"""Maintainable ParcelBook R2/DuckDB parcel-search builder.

This module is the project version of the Colab experiment in
``data/openskagit_parcel_search_builder.py``. It keeps the same one-row-per-parcel
derived outputs while sourcing R2 credentials from environment variables.
"""
...
class OpenSkagitParcelSearchBuilder:
    ...
    def __post_init__(self) -> None:
        self.bucket = self.bucket or os.environ.get("R2_BUCKET", "openskagit")
        self.account_id = self.account_id or os.environ.get("R2_ACCOUNT_ID")
        self.key_id = self.key_id or os.environ.get("R2_ACCESS_KEY_ID")
        self.secret = self.secret or os.environ.get("R2_SECRET_ACCESS_KEY")
        if self.con is None:
            self.con = duckdb.connect()

    @classmethod
    def from_env(cls) -> "OpenSkagitParcelSearchBuilder":
        return cls()

    def _require_r2_credentials(self) -> None:
        missing = [
            name
            for name, value in {
    ...
    def connect(self) -> None:
        ...
        CREATE OR REPLACE SECRET r2_secret (
            TYPE R2,
            KEY_ID '{self._sql_string(self.key_id)}',
            SECRET '{self._sql_string(self.secret)}',

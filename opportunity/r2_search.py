...
- For absentee-owner, nonresident-owner, or mailing-address-differs-from-site requests, join r2://openskagit/assessor.parquet as a to derived/parcel_search.parquet as ps on ps.parcel_number = TRIM(a."Parcel Number").
- Compare normalized address keys: lower-case and remove punctuation/whitespace with regexp_replace(..., '[^a-z0-9]+', '', 'g'). Build the situs key from ps.situs_address and the mailing key from concat_ws(' ', a."Owner Add 1", a."Owner Add 2", a."Owner Add 3", a."Owner City", a."Owner State", a."Owner Zip").
- Treat absentee ownership as both normalized keys being non-empty and different. Keep this as a hard WHERE condition when the user requests absentee owners, and add match_reasons such as 'owner mailing address differs from situs address'.
